...
module.exports.CreateMeshAgent = function (parent, db, ws, req, args, domain) {
    ...
    if (args.agentidletimeout != 0) {
        ws._socket.setTimeout(args.agentidletimeout, function () {
            obj.close(1);
        });
    } // Inactivity timeout of 2:30 minutes, by default agent will WebSocket ping every 2 minutes and server will pong back.
    //obj.nodeid = null;
    //obj.meshid = null;
    //obj.dbNodeKey = null;
    //obj.dbMeshKey = null;
    ...
    obj.send = function (data, func) {
        try {
            if (typeof data == 'string') {
                ws.send(Buffer.from(data), func);
            } else {
                ws.send(data, func);
            }
        } catch (e) {
    ...
    obj.sendBinary = function (data, func) {
        try {
            if (typeof data == 'string') {
                ws.send(Buffer.from(data, 'binary'), func);
            } else {
                ws.send(data, func);
            }
        } catch (e) {
    ...
    obj.close = function (arg) {
        ...
        if ((arg == 1) || (arg == null)) {
            try {
                ws.close();
                if (obj.nodeid != null) {
                    parent.parent.debug('agent', 'Soft disconnect ' + obj.nodeid + ' (' + obj.remoteaddrport + ')');
                }
            } catch (e) {
                console.log(e);
            }
        ...
        if (parent.wsagents[obj.dbNodeKey] == obj) {
            ...
            parent.parent.ClearConnectivityState(obj.dbMeshKey, obj.dbNodeKey, 1, null, {
                remoteaddrport: obj.remoteaddrport,
                name: obj.name
            });
        }
        ...
        if (obj.badWebCert) {
            delete parent.wsagentsWithBadWebCerts[obj.badWebCert];
        }
        ...
        if (((obj.agentInfo) && (obj.agentInfo.capabilities) && ((obj.agentInfo.capabilities & 0x20) || (obj.agentInfo.capabilities & 0x40))) || ((mesh) && (mesh.flags) && (mesh.flags & 1))) {
            ...
            // Event node deletion
            parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(obj.dbMeshKey, [obj.dbNodeKey]), obj, {
                etype: 'node',
                action: 'removenode',
            ...
            if ((state != null) && (state.connectivity != null)) {
                if ((state.connectivity & 1) != 0) {
                    parent.wsagents[obj.dbNodeKey].close();
                } // Disconnect mesh agent
                if ((state.connectivity & 2) != 0) {
                    parent.parent.mpsserver.closeAllForNode(obj.dbNodeKey);
                } // Disconnect CIRA connection
            }
        }
        ...
        if (obj.pingtimer) {
            clearInterval(obj.pingtimer);
            delete obj.pingtimer;
        }
        if (obj.pongtimer) {
            clearInterval(obj.pongtimer);
            delete obj.pongtimer;
        }
    ...
    ws.on('message', function (msg) {
        ...
        if (obj.authenticated == 2) { // We are authenticated
            if ((obj.agentUpdate == null) && (msg.charCodeAt(0) == 123)) {
                processAgentData(msg);
            } // Only process JSON messages if meshagent update is not in progress
            if (msg.length < 2) return;
            const cmdid = common.ReadShort(msg, 0);
            if (cmdid == 11) { // MeshCommand_CoreModuleHash
                if (msg.length == 4) {
                    ChangeAgentCoreInfo({'caps': 0});
                } // If the agent indicated that no core is running, clear the core information string.
                    ...
                    if (agentMeshCoreHash != meshcorehash) {
                        if ((obj.agentCoreCheck < 5) || (obj.agentCoreCheck == 1001) || (obj.agentCoreCheck == 1011) || (obj.agentCoreUpdate == true)) {
                            if (meshcorehash == null) {
                                ...
                                var taskLimiterOptions = {
                                    hash: meshcorehash,
                                    core: parent.parent.defaultMeshCores[corename],
                                    name: corename
                                };
                                ...
                                parent.parent.taskLimiter.launch(function (argument, taskid, taskLimiterQueue) {
                                    if (obj.authenticated == 2) {
                                        ...
                                        obj.sendBinary(common.ShortToStr(10) + common.ShortToStr(0) + argument.hash + argument.core.toString('binary'), function () {
                                            parent.parent.taskLimiter.completed(taskid);
                                        }); // MeshCommand_CoreModule, start core update
                ...
                */
                ...
                if ((msg.length == 52) && (obj.agentExeInfo != null) && (obj.agentExeInfo.update == true)) {
                    ...
                    if (agentUpdateMethod === 2) { // Use meshcore agent update system
                        ...
                        parent.parent.taskLimiter.launch(function (argument, taskid, taskLimiterQueue) {
                            ...
                            if (obj.nodeid != null) {
                                parent.parent.debug('agent', "Agent update required, NodeID=0x" + obj.nodeid.substring(0, 16) + ', ' + obj.agentExeInfo.desc);
                            }
                        ...
                        if (obj.agentExeInfo.data == null) {
                            ...
                            parent.fs.read(obj.agentUpdate.fd, obj.agentUpdate.buf, 4, parent.parent.agentUpdateBlockSize, obj.agentUpdate.ptr, function (err, bytesRead, buffer) {
                                ...
                                if ((err != null) || (bytesRead < 0)) {
                                    ...
                                    if (bytesRead == parent.parent.agentUpdateBlockSize) {
                                        ...
                                        obj.sendBinary(obj.agentUpdate.buf.slice(0, bytesRead + 4));
                                    } // Command 14, mesh agent next data block
                            ...
                            if (len > 0) {
                                ...
                                if (len == parent.parent.agentUpdateBlockSize) {
                                    ...
                                    obj.sendBinary(obj.agentUpdate.buf.slice(0, len + 4));
                                } // Command 14, mesh agent next data block
                ...
                while (tag.charCodeAt(tag.length - 1) == 0) {
                    tag = tag.substring(0, tag.length - 1);
                } // Remove end-of-line zeros.
                ChangeAgentTag(tag);
            }
            ...
            if (msg.charCodeAt(0) == 123) {
                ...
                if (str[0] == '{') {
                    ...
                    if ((command != null) && (command.action === 'agentName') && (typeof command.value == 'string') && (command.value.length > 0) && (command.value.length < 256)) {
                        obj.agentName = command.value;
                    }
                }
                return;
            }
            ...
            if (cmd == 1) {
                ...
                if (obj.unauthsign != null) {
                    if (processAgentSignature(obj.unauthsign) == false) {
                        ...
                        console.log('Agent connected with bad signature, holding connection (' + obj.remoteaddrport + ').');
                        return;
                    } else {
                ...
                obj.unauth = {};
                try {
                    obj.unauth.nodeid = Buffer.from(forge.pki.getPublicKeyFingerprint(forge.pki.certificateFromAsn1(forge.asn1.fromDer(msg.substring(4, 4 + certlen))).publicKey, {md: forge.md.sha384.create()}).data, 'binary').toString('base64').replace(/\+/g, '@').replace(/\//g, '$');
                } catch (ex) {
                    console.log(ex);
                    parent.parent.debug('agent', ex);
                    return;
                }
                ...
                if (obj.agentInfo.platformType > 8 || obj.agentInfo.platformType < 1) {
                    obj.agentInfo.platformType = 1;
                }
                ...
                if ((msg.substring(2, 34) == parent.swarmCertificateHash256) || (msg.substring(2, 50) == parent.swarmCertificateHash384)) {
                    obj.useSwarmCert = true;
                }
            } else if (cmd == 30) {
                // Agent Commit Date. This is future proofing. Can be used to change server behavior depending on the date range of the agent.
                try {
                    obj.AgentCommitDate = Date.parse(msg.substring(2))
                } catch (ex) {
                }
    ...
    ws.on('error', function (err) {
        parent.parent.debug('agent', 'AGENT WSERR: ' + err);
        console.log('AGENT WSERR: ' + err);
        obj.close(0);
    });
    ...
    function getMeshAutoCreate() {
        ...
        if ((mesh == null) && (typeof domain.orphanagentuser == 'string')) {
            ...
            if ((adminUser != null) && (adminUser.siteadmin == 0xFFFFFFFF)) {
                ...
                parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(obj.dbMeshKey, [adminUser._id, obj.dbNodeKey]), obj, {
                    etype: 'mesh',
                    username: adminUser.name,
                    meshid: obj.dbMeshKey,
                    name: meshname,
                    mtype: 2,
                    desc: '',
                    action: 'createmesh',
                    ...
                    msgid: 55,
                    msgArgs: [obj.meshid],
                    msg: "Created device group: " + obj.meshid,
                    domain: domain.id
                });
            }
        } else {
            if ((mesh != null) && (mesh.deleted != null) && (mesh.links)) {
                ...
                parent.parent.DispatchEvent(ids, obj, {
                    etype: 'mesh',
                    meshid: mesh._id,
                    name: mesh.name,
                    mtype: mesh.mtype,
                    ...
                    msgid: 56,
                    msgArgs: [mesh._id],
                    msg: "Device group undeleted: " + mesh._id,
                    domain: domain.id
                });
    ...
    function sendPing() {
        obj.send('{"action":"ping"}');
    }

    function sendPong() {
        obj.send('{"action":"pong"}');
    }
    ...
    function completeAgentConnection() {
        ...
        if ((typeof args.agentping == 'number') && (obj.pingtimer == null)) {
            obj.pingtimer = setInterval(sendPing, args.agentping * 1000);
        } else if ((typeof args.agentpong == 'number') && (obj.pongtimer == null)) {
            obj.pongtimer = setInterval(sendPong, args.agentpong * 1000);
        }
        ...
        if (typeof domain.limits.maxagentsessions == 'number') {
            ...
            for (var i in parent.wsagents) {
                if (parent.wsagents[i].domain.id == domain.id) {
                    domainAgentSessionCount++;
                }
            }
        ...
        db.Get(obj.dbNodeKey, function (err, nodes) {
            ...
            if ((nodes == null) || (nodes.length == 0)) {
                ...
                if (device.agent == null) {
                    device.agent = {
                        ver: obj.agentInfo.agentVersion,
                        id: obj.agentInfo.agentId,
                        caps: obj.agentInfo.capabilities
                    };
                    change = 1;
                }
                if (device.rname != obj.agentInfo.computerName) {
                    device.rname = obj.agentInfo.computerName;
                    change = 1;
                    changes.push('computer name');
                }
                if (device.agent.ver != obj.agentInfo.agentVersion) {
                    device.agent.ver = obj.agentInfo.agentVersion;
                    change = 1;
                    changes.push('agent version');
                }
                if (device.agent.id != obj.agentInfo.agentId) {
                    device.agent.id = obj.agentInfo.agentId;
                    change = 1;
                    changes.push('agent type');
                }
                if ((device.agent.caps & 24) != (obj.agentInfo.capabilities & 24)) {
                    device.agent.caps = obj.agentInfo.capabilities;
                    change = 1;
                    changes.push('agent capabilities');
                } // If agent console or javascript support changes, update capabilities
                ...
                if (device.ip != obj.remoteaddr) {
                    device.ip = obj.remoteaddr;
                    change = 1;
                }
                ...
                if (change == 1) {
                    ...
                    if (log == 0) {
                        ...
                        event.msg = 'Changed device ' + device.name + ' from group ' + mesh.name + ': ' + changes.join(', ');
                    }
                    if (db.changeStream) {
                        event.noact = 1;
    ...
    function completeAgentConnection2() {
        ...
        var device = {
            ...
            mtype: mesh.mtype,
            _id: obj.dbNodeKey,
            icon: obj.agentInfo.platformType,
            meshid: obj.dbMeshKey,
            name: agentName,
            rname: obj.agentInfo.computerName,
            domain: domain.id,
            agent: {ver: obj.agentInfo.agentVersion, id: obj.agentInfo.agentId, caps: obj.agentInfo.capabilities},
            host: null,
            firstconnect: obj.connectTime
        };
        if ((obj.deviceInfo != null) && (typeof obj.deviceInfo.uuid == 'string') && (obj.deviceInfo.uuid.length > 0)) {
            device.deviceuuid = obj.deviceInfo.uuid;
        }
        db.Set(device);
        ...
        if (obj.agentInfo.capabilities & 0x20) {
            ...
            parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(obj.dbMeshKey, [obj.dbNodeKey]), obj, {
                ...
                node: device,
                msgid: 57,
                msgArgs: [obj.agentInfo.computerName, mesh.name],
                msg: ('Added device ' + obj.agentInfo.computerName + ' to device group ' + mesh.name),
                domain: domain.id
            });
        }
    ...
    function completeAgentConnection3(device, mesh) {
        ...
        if (dupAgent) {
            ...
            if (parent.duplicateAgentsLog[obj.dbNodeKey] == null) {
                if (dupAgent.remoteaddr == obj.remoteaddr) {
                    parent.duplicateAgentsLog[obj.dbNodeKey] = {
                        name: device.name,
                        group: mesh.name,
                        ip: [obj.remoteaddr],
                        count: 1
                    ...
                    parent.duplicateAgentsLog[obj.dbNodeKey] = {
                        name: device.name,
                        group: mesh.name,
                        ip: [obj.remoteaddr, dupAgent.remoteaddr],
                        count: 1
                    };
                }
            } else {
                ...
                if (parent.duplicateAgentsLog[obj.dbNodeKey].ip.indexOf(obj.remoteaddr) == -1) {
                    parent.duplicateAgentsLog[obj.dbNodeKey].ip.push(obj.remoteaddr);
                }
            }
            ...
            if (obj.nodeid != null) {
                parent.parent.debug('agent', 'Duplicate agent ' + obj.nodeid + ' (' + obj.remoteaddrport + ')');
            }
            dupAgent.close(3);
        } else {
            // Indicate the agent is connected
            parent.parent.SetConnectivityState(obj.dbMeshKey, obj.dbNodeKey, obj.connectTime, 1, 1, null, {
                remoteaddrport: obj.remoteaddrport,
                name: device.name
            });
        }
        ...
        if ((obj.agentInfo == null) || (typeof obj.agentInfo.capabilities != 'number')) {
            return;
        } // This is an odd case.
        obj.agentExeInfo = parent.parent.meshAgentBinaries[obj.agentInfo.agentId];
        if (domain.meshAgentBinaries && domain.meshAgentBinaries[obj.agentInfo.agentId]) {
            obj.agentExeInfo = domain.meshAgentBinaries[obj.agentInfo.agentId];
        }
    ...
    obj.sendUpdatedIntelAmtPolicy = function (policy) {
        if (obj.agentExeInfo && (obj.agentExeInfo.amt == true)) { // Only send Intel AMT policy to agents what could have AMT.
            ...
            if ((policy != null) && (policy.type != 0)) {
                const cookie = parent.parent.encodeCookie({
                    a: 'apf',
                    n: obj.dbNodeKey,
                    m: obj.dbMeshKey
                }, parent.parent.loginCookieEncryptionKey);
                try {
                    obj.send(JSON.stringify({action: 'amtconfig', user: '**MeshAgentApfTunnel**', pass: cookie}));
                } catch (ex) {
                }
            }
        }
    ...
    function recoveryAgentCoreIsStable(mesh) {
        ...
        db.Get('da' + obj.dbNodeKey, function (err, nodes, self) {
            if ((nodes != null) && (nodes.length == 1)) {
                ...
                if (state) {
                    agentConnected = ((state.connectivity & 1) != 0)
                }

                self.send(JSON.stringify({
                    action: 'diagnostic',
                    value: {command: 'query', value: self.realNodeKey, agent: agentConnected}
                }));
            } else {
    ...
    function agentCoreIsStable() {
        ...
        db.Get('ra' + obj.dbNodeKey, function (err, nodes) {
            if ((nodes != null) && (nodes.length == 1)) {
                ...
                obj.send(JSON.stringify({
                    action: 'diagnostic',
                    value: {command: 'query', value: obj.diagnosticNodeKey}
                }));
            }
        });
        ...
        if ((parent.parent.config.settings != null) && ((parent.parent.config.settings.agentcoredump === true) || (parent.parent.config.settings.agentcoredump === false))) {
            ...
            if (parent.parent.config.settings.agentcoredump === true) {
                ...
                if ((parent.lastCoreDumpRequest == null) || ((Date.now() - parent.lastCoreDumpRequest) >= 60000)) {
                    obj.send(JSON.stringify({action: 'getcoredump'}));
                }
            }
        }
        ...
        if ((typeof domain.terminal == 'object') && (typeof domain.terminal.launchcommand == 'object')) {
            ...
            if (typeof domain.terminal.launchcommand.linux == 'string') {
                serverInfo.termlaunchcommand.linux = domain.terminal.launchcommand.linux;
            }
            if (typeof domain.terminal.launchcommand.darwin == 'string') {
                serverInfo.termlaunchcommand.darwin = domain.terminal.launchcommand.darwin;
            }
            if (typeof domain.terminal.launchcommand.freebsd == 'string') {
                serverInfo.termlaunchcommand.freebsd = domain.terminal.launchcommand.freebsd;
            }
        }
        // Enable agent self guest sharing if allowed
        if (domain.agentselfguestsharing) {
            serverInfo.agentSelfGuestSharing = true;
        }
    ...
    function processAgentSignature(msg) {
        if (isIgnoreHashCheck() == false) {
            ...
            if ((msg.length != 384) && (msg.length != 256)) {
                ...
                var msgDer = null;
                try {
                    msgDer = forge.asn1.fromDer(forge.util.createBuffer(msg, 'binary'));
                } catch (ex) {
                }
                if (msgDer != null) {
                    try {
            ...
            switch (command.action) {
                case 'deviceinfo': {
                    if ((typeof command.uuid == 'string') && (command.uuid.length > 0) && (command.uuid.length < 256)) {
                        ...
                        db.Get(obj.dbNodeKey, function (err, nodes) {
                            ...
                            const node = nodes[0];
                            if (node.deviceuuid === command.uuid) return;
                ...
                case 'msg': {
                    ...
                    if (command.type == 'console') {
                        if (obj.consoleKickValue == command.value) {
                            if (obj.consoleKickCount) {
                                obj.consoleKickCount++;
                            } else {
                                obj.consoleKickCount = 1;
                            }
                            if (obj.consoleKickCount > 30) {
                                obj.close();
                                return;
                            } // 30 identical console messages received, kick out this agent.
                        } else {
                ...
                case 'coreinfo': {
                    ...
                    if ((obj.agentCoreUpdate === true) && (obj.agentExeInfo != null) && (typeof obj.agentExeInfo.url == 'string')) {
                        ...
                        parent.parent.taskLimiter.launch(function (argument, taskid, taskLimiterQueue) { // Medium priority task
                            ...
                            // Add the hash
                            if (obj.agentExeInfo.fileHash != null) {
                            ...
                            if (isIgnoreHashCheck() == false) {
                                ...
                                if (tlsCertHash != null) {
                                    cmd.servertlshash = Buffer.from(tlsCertHash, 'binary').toString('hex');
                                }
                            }
                ...
                case 'smbios': {
                    ...
                    if (domain.smbios === true) {
                        ...
                        try {
                            for (var i in command.value) {
                                var k = parseInt(i);
                                if ((k != i) || (i > 255) || (typeof command.value[i] != 'object') || (command.value[i].length == null) || (command.value[i].length > 1024) || (command.value[i].length < 0)) {
                                    delete command.value[i];
                                }
                            }
                            db.SetSMBIOS({
                                _id: obj.dbNodeKey,
                ...
                case 'netinfo': {
                    ...
                    if (command.netif2 != null) {
                        for (var i in command.netif2) {
                            ...
                            if (esc !== i) {
                                command.netif2[esc] = command.netif2[i];
                                delete command.netif2[i];
                            }
                        }
                    }
                    ...
                    parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(obj.meshid, [obj.dbNodeKey]), obj, {
                        action: 'ifchange',
                        nodeid: obj.dbNodeKey,
                        domain: domain.id,
                        nolog: 1
                    ...
                    db.Get(oldNodeKey, function (err, nodes) {
                        ...
                        if (node.meshid == obj.dbMeshKey) {
                            ...
                            if (node.intelamt != null) {
                                newNode.intelamt = node.intelamt;
                            }
                            ...
                            parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(node.meshid, [obj.dbNodeKey]), obj, {
                                etype: 'node',
                                action: 'removenode',
                                nodeid: node._id,
                                msg: change,
                ...
                case 'log': {
                    ...
                    if ((typeof command.msg == 'string') && (command.msg.length < 4096)) {
                        ...
                        if (typeof command.msgid == 'number') {
                            event.msgid = command.msgid;
                        }
                        if (typeof command.guestname == 'string') {
                            event.guestname = command.guestname;
                        }
                        if (Array.isArray(command.msgArgs)) {
                            event.msgArgs = command.msgArgs;
                        }
                        if (typeof command.remoteaddr == 'string') {
                            event.remoteaddr = command.remoteaddr;
                        }
                        ...
                        if (typeof command.userid == 'string') {
                            ...
                            if (loguser) {
                                ...
                                event.username = loguser.name;
                                targets.push(command.userid);
                            }
                        }
                        if (typeof command.xuserid == 'string') {
                            var xloguser = parent.users[command.xuserid];
                            if (xloguser) {
                                targets.push(command.xuserid);
                            }
                        }
                        if ((typeof command.sessionid == 'string') && (command.sessionid.length < 500)) {
                        ...
                        if (event.msgid == 98) {
                            ...
                            db.Get(obj.dbNodeKey, function (err, nodes) { // TODO: THIS IS A BIG RACE CONDITION HERE, WE NEED TO FIX THAT. If this call is made twice at the same time on the same device, data will be missed.
                                ...
                                if (typeof device.name == 'string') {
                                    parent.parent.NotifyUserOfDeviceHelpRequest(domain, device.meshid, device._id, device.name, command.msgArgs[0], command.msgArgs[1]);
                                }
                            });
                        }
                    }
                            ...
                            case 'register': {
                                ...
                                if (((obj.agentInfo.capabilities & 0x40) == 0) && (typeof command.value.value == 'string') && (command.value.value.length == 64)) {
                                    ...
                                    db.Set({
                                        _id: 'da' + daNodeKey,
                                        domain: domain.id,
                                        time: obj.connectTime,
                                        raid: obj.dbNodeKey
                                    });  // DiagnosticAgent --> Agent
                                    db.Set({
                                        _id: 'ra' + obj.dbNodeKey,
                                        domain: domain.id,
                                        time: obj.connectTime,
                                        daid: daNodeKey
                                ...
                                if ((obj.agentInfo.capabilities & 0x40) != 0) {
                                    ...
                                    db.Get('da' + obj.dbNodeKey, function (err, nodes) {
                                        if ((nodes != null) && (nodes.length == 1)) {
                                            ...
                                            obj.send(JSON.stringify({
                                                action: 'diagnostic',
                                                value: {command: 'query', value: obj.realNodeKey, agent: agentConnected}
                                            }));
                                        } else {
                                            obj.send(JSON.stringify({
                                                action: 'diagnostic',
                                                value: {command: 'query', value: null}
                                            }));
                                        }
                                    });
                                }
                            ...
                            case 'log': {
                                if (((obj.agentInfo.capabilities & 0x40) != 0) && (typeof command.value.value == 'string') && (command.value.value.length < 256)) {
                                    ...
                                    var event = {
                                        etype: 'node',
                                        action: 'diagnostic',
                                        nodeid: obj.realNodeKey,
                                        snodeid: obj.dbNodeKey,
                                        domain: domain.id,
                ...
                case 'sessions': {
                    ...
                    if (typeof command.value != null) {
                        if (command.type == 'kvm') {
                            obj.sessions.kvm = command.value;
                        } else if (command.type == 'terminal') {
                            obj.sessions.terminal = command.value;
                        } else if (command.type == 'files') {
                            obj.sessions.files = command.value;
                        } else if (command.type == 'help') {
                            obj.sessions.help = command.value;
                        } else if (command.type == 'tcp') {
                            obj.sessions.tcp = command.value;
                        } else if (command.type == 'udp') {
                            obj.sessions.udp = command.value;
                        } else if (command.type == 'msg') {
                            obj.sessions.msg = command.value;
                        } else if (command.type == 'app') {
                            obj.sessions.app = command.value;
                        }
                    }

                    // Any "help" session must have an associated app, if not, remove it.
                    if (obj.sessions.help != null) {
                        for (var i in obj.sessions.help) {
                            if (obj.sessions.help[i] == null) {
                                delete obj.sessions.help[i];
                            }
                        }
                        if (Object.keys(obj.sessions.help).length == 0) {
                            delete obj.sessions.help;
                        }
                    }
                ...
                case 'battery': {
                    ...
                    if (obj.sessions.battery == null) {
                        obj.sessions.battery = {};
                    }
                    if ((command.state == 'ac') || (command.state == 'dc')) {
                        obj.sessions.battery.state = command.state;
                    } else {
                        delete obj.sessions.battery.state;
                    }
                    if ((typeof command.level == 'number') && (command.level >= 0) && (command.level <= 100)) {
                        obj.sessions.battery.level = command.level;
                    } else {
                        delete obj.sessions.battery.level;
                    }
                    obj.updateSessions();
                    break;
                }
                ...
                case 'tunnelCloseStats': {
                    ...
                    var event = {
                        etype: 'node',
                        action: 'sessioncompression',
                        nodeid: obj.dbNodeKey,
                        domain: domain.id,
                        sent: parseInt(command.sent),
                        sentActual: parseInt(command.sentActual),
                        msgid: 54,
                        msgArgs: [command.sentRatio, parseInt(command.sent), parseInt(command.sentActual)],
                        msg: 'Agent closed session with ' + command.sentRatio + '% agent to server compression. Sent: ' + command.sent + ', Compressed: ' + command.sentActual + '.'
                    };
                    parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(obj.dbMeshKey, [obj.dbNodeKey]), obj, event);
                    break;
                }
                ...
                case 'meshToolInfo': {
                    ...
                    if ((command.name == 'MeshCentralAssistant') && (command.msh == true)) {
                        responseCmd.url = '*/' + xdomain + 'meshagents?id=10006';
                    } // If this is Assistant and the MSH needs to be included in the executable, change the URL.
                    if (command.cookie === true) {
                        responseCmd.url += ('&auth=' + parent.parent.encodeCookie({download: info.dlname}, parent.parent.loginCookieEncryptionKey));
                    }
                    if (command.pipe === true) {
                        responseCmd.pipe = true;
                    }
                    if (parent.webCertificateHashs[domain.id] != null) {
                        responseCmd.serverhash = Buffer.from(parent.webCertificateHashs[domain.id], 'binary').toString('hex');
                    }
                    try {
                        ws.send(JSON.stringify(responseCmd));
                    } catch (ex) {
                    }
                    break;
                }
                    ...
                    if ((obj.agentExeInfo != null) && (typeof obj.agentExeInfo.url == 'string')) {
                        var func = function agentUpdateFunc(argument, taskid, taskLimiterQueue) { // Medium priority task
                            ...
                            var cmd = {
                                action: 'agentupdate',
                                url: url,
                                hash: obj.agentExeInfo.hashhex,
                                sessionid: agentUpdateFunc.sessionid
                            };
                ...
                case '2faauth': {
                    ...
                    if (authCookie.a == 'addAuth') {
                        ...
                        var event = {
                            ...
                            userid: user._id,
                            username: user.name,
                            account: parent.CloneSafeUser(user),
                            action: 'accountchange',
                            msgid: 113,
                            msg: "Added push notification authentication device",
                            domain: domain.id
                        };
                        if (db.changeStream) {
                            event.noact = 1;
                    ...
                    if (authCookie.a == 'checkAuth') {
                        ...
                        // Notify 2FA response
                        parent.parent.DispatchEvent(['2fadev-' + authCookie.s], obj, {
                            etype: '2fadev',
                            action: '2faresponse',
                            domain: domain.id,
                            nodeid: obj.dbNodeKey,
                ...
                case 'getUserImage': {
                    ...
                    if (typeof command.userid != 'string') {
                        ...
                        if ((command.default) || (command.sentDefault)) {
                            try {
                                command.image = 'data:image/png;base64,' + Buffer.from(parent.fs.readFileSync(parent.parent.path.join(__dirname, 'public', 'images', 'user-128.png')), 'binary').toString('base64');
                            } catch (ex) {
                            }
                ...
                case 'getServerImage': {
                    if (command.agent === 'assistant') {
                        ...
                        if ((domain.assistantcustomization != null) && (typeof domain.assistantcustomization == 'object')) {
                            ...
                            if (typeof domain.assistantcustomization.title == 'string') {
                                ok = true;
                                command.title = domain.assistantcustomization.title;
                            }
                            if (typeof domain.assistantcustomization.image == 'string') {
                                try {
                                    command.image = 'data:image/jpeg;base64,' + Buffer.from(parent.fs.readFileSync(parent.parent.getConfigFilePath(domain.assistantcustomization.image)), 'binary').toString('base64');
                                    ok = true;
                                } catch (ex) {
                                    console.log(ex);
                                }
                    ...
                    if (command.agent === 'android') {
                        ...
                        if ((domain.androidcustomization != null) && (typeof domain.androidcustomization == 'object')) {
                            var ok = false;
                            if (typeof domain.androidcustomization.title == 'string') {
                                ok = true;
                                command.title = domain.androidcustomization.title;
                            }
                            if (typeof domain.androidcustomization.subtitle == 'string') {
                                ok = true;
                                command.subtitle = domain.androidcustomization.subtitle;
                            }
                            if (typeof domain.androidcustomization.image == 'string') {
                                try {
                                    command.image = 'data:image/jpeg;base64,' + Buffer.from(parent.fs.readFileSync(parent.parent.getConfigFilePath(domain.androidcustomization.image)), 'binary').toString('base64');
                                    ok = true;
                                } catch (ex) {
                                    console.log(ex);
                                }
                ...
                case 'guestShare': {
                    if ((command.flags == null) || (command.flags == 0)) {
                        ...
                        if ((command.flags & 2) == 0) {
                            command.viewOnly = false;
                        } // Only allow "view only" if desktop is shared.
                ...
                case 'script-task': {
                    ...
                    if (parent.parent.taskManager != null) {
                        parent.parent.taskManager.agentAction(command, obj);
                    }
                    break;
                }
                default: {
    ...
    function addGuestSharing(flags, viewOnly, func) {
        ...
        if (serverName.split('.') == 1) {
            url = '/' + xdomain + page + '?c=' + inviteCookie;
        }

        // Create a device sharing database entry
        var shareEntry = {
            _id: 'deviceshare-' + publicid,
            type: 'deviceshare',
            nodeid: obj.dbNodeKey,
            p: flags,
            domain: domain.id,
        ...
        var event = {
            etype: 'node',
            nodeid: obj.dbNodeKey,
            action: 'addedDeviceShare',
            msg: 'Added device share with unlimited time',
            msgid: 131,
            msgArgs: ['Agent'],
            domain: domain.id
        };
        ...
        parent.db.GetAllTypeNodeFiltered([obj.dbNodeKey], domain.id, 'deviceshare', null, function (err, docs) {
            ...
            // Callback
            if (func) {
                func({url: url, flags: flags, viewOnly: viewOnly});
            }
        });
    }
        ...
        parent.db.GetAllTypeNodeFiltered([obj.dbNodeKey], domain.id, 'deviceshare', null, function (err, docs) {
            ...
            if (removedExact != null) {
                ...
                var event = {
                    etype: 'node',
                    nodeid: obj.dbNodeKey,
                    action: 'removedDeviceShare',
                    msg: 'Removed Device Share',
                    msgid: 102,
                    msgArgs: ['Agent'],
                    domain: domain.id,
                    publicid: publicid
    ...
    obj.updateSessions = function () {
        ...
        for (var i in obj.sessions) {
            if (Object.keys(obj.sessions[i]).length == 0) {
                delete obj.sessions[i];
            }
        }
    ...
    function ChangeAgentCoreInfo(command) {
        ...
        db.Get(obj.dbNodeKey, function (err, nodes) { // TODO: THIS IS A BIG RACE CONDITION HERE, WE NEED TO FIX THAT. If this call is made twice at the same time on the same device, data will be missed.
            ...
            if (device.agent) {
                ...
                if (command.name && (typeof command.name == 'string') && (command.name != device.name)) {
                    change = 1;
                    log = 1;
                    device.name = command.name;
                    changes.push('name');
                }
                if ((command.caps != null) && (device.agent.core != command.value)) {
                    if ((command.value == null) && (device.agent.core != null)) {
                        delete device.agent.core;
                    } else {
                        device.agent.core = command.value;
                    }
                    change = 1;
                } // Don't save this as an event to the db.
                if ((command.caps != null) && ((device.agent.caps & 0xFFFFFFE7) != (command.caps & 0xFFFFFFE7))) {
                    device.agent.caps = ((device.agent.caps & 24) + (command.caps & 0xFFFFFFE7));
                    change = 1;
                } // Allow Javascript on the agent to change all capabilities except console and javascript support, Don't save this as an event to the db.
                if ((command.osdesc != null) && (typeof command.osdesc == 'string') && (device.osdesc != command.osdesc)) {
                    device.osdesc = command.osdesc;
                    change = 1;
                    changes.push('os desc');
                } // Don't save this as an event to the db.
                if ((typeof command.root == 'boolean') && (command.root !== device.agent.root)) {
                    change = 1;
                    device.agent.root = command.root;
                }
                if (device.ip != obj.remoteaddr) {
                    device.ip = obj.remoteaddr;
                    change = 1;
                ...
                if (command.intelamt) {
                    ...
                    if ((command.intelamt.Versions != null) && (typeof command.intelamt.Versions == 'object')) {
                        if ((command.intelamt.Versions.AMT != null) && (typeof command.intelamt.Versions.AMT == 'string') && (command.intelamt.Versions.AMT.length < 12) && (device.intelamt.ver != command.intelamt.Versions.AMT)) {
                            changes.push('AMT version');
                            device.intelamt.ver = command.intelamt.Versions.AMT;
                            change = 1;
                            log = 1;
                        }
                        if ((command.intelamt.Versions.Sku != null) && (typeof command.intelamt.Versions.Sku == 'string')) {
                            ...
                            if (device.intelamt.sku !== sku) {
                                device.intelamt.sku = sku;
                                change = 1;
                                log = 1;
                            }
                        }
                    }
                    if ((command.intelamt.ProvisioningState != null) && (typeof command.intelamt.ProvisioningState == 'number') && (device.intelamt.state != command.intelamt.ProvisioningState)) {
                        changes.push('AMT state');
                        device.intelamt.state = command.intelamt.ProvisioningState;
                        change = 1;
                        log = 1;
                    }
                    if ((command.intelamt.Flags != null) && (typeof command.intelamt.Flags == 'number') && (device.intelamt.flags != command.intelamt.Flags)) {
                        if (device.intelamt.flags) {
                            changes.push('AMT flags (' + device.intelamt.flags + ' --> ' + command.intelamt.Flags + ')');
                        } else {
                            changes.push('AMT flags (' + command.intelamt.Flags + ')');
                        }
                        device.intelamt.flags = command.intelamt.Flags;
                        change = 1;
                        log = 1;
                    }
                    if ((command.intelamt.UUID != null) && (typeof command.intelamt.UUID == 'string') && (device.intelamt.uuid != command.intelamt.UUID)) {
                        changes.push('AMT uuid');
                        device.intelamt.uuid = command.intelamt.UUID;
                        change = 1;
                        log = 1;
                    }
                }
                if (command.av != null) { // Antivirus
                    ...
                    if (JSON.stringify(device.av) != JSON.stringify(command.av)) { /*changes.push('AV status');*/
                        device.av = command.av;
                        change = 1;
                        log = 1;
                    }
                }
                if (command.wsc != null) { // Windows Security Center
                    ...
                    if (JSON.stringify(device.wsc) != JSON.stringify(command.wsc)) { /*changes.push('Windows Security Center status');*/
                        device.wsc = command.wsc;
                        change = 1;
                        log = 1;
                    }
                }
                if (command.defender != null) { // Defender For Windows Server
                    ...
                    if (JSON.stringify(device.defender) != JSON.stringify(command.defender)) { /*changes.push('Defender status');*/
                        device.defender = command.defender;
                        change = 1;
                        log = 1;
                    }
                }
                if (command.lastbootuptime != null) { // Last Boot Up Time
                    if (!device.lastbootuptime) {
                        device.lastbootuptime = "";
                    }
                    if (device.lastbootuptime != command.lastbootuptime) { /*changes.push('Last Boot Up Time');*/
                        device.lastbootuptime = command.lastbootuptime;
                        change = 1;
                        log = 1;
                    }
                }
                if (command.idletime != null) { // Idle Time
                    ...
                    if (parseInt(device.idletime) != parseInt(command.idletime)) { /*changes.push('Idle Time');*/
                        device.idletime = parseInt(command.idletime);
                        change = 1;
                    } // Don't log idle time changes, this is too volatile.
                }

                // Push Messaging Token
                if ((command.pmt != null) && (typeof command.pmt == 'string') && (device.pmt != command.pmt)) {
                    if (typeof device.pmt == 'string') {
                        db.Remove('pmt_' + device.pmt);
                    }
                ...
                if ((command.users != null) && (Array.isArray(command.users)) && (device.users != command.users)) {
                    device.users = command.users;
                    change = 1;
                } // Don't save this to the db.
                if ((command.lusers != null) && (Array.isArray(command.lusers)) && (device.lusers != command.lusers)) {
                    device.lusers = command.lusers;
                    change = 1;
                } // Don't save this to the db.
                if ((command.upnusers != null) && (Array.isArray(command.upnusers)) && (device.upnusers != command.upnusers)) {
                    device.upnusers = command.upnusers;
                    change = 1;
                } // Don't save this to the db.
                ...
                if (change == 1) {
                    ...
                    if ((log == 0) || ((obj.agentInfo) && (obj.agentInfo.capabilities) && (obj.agentInfo.capabilities & 0x20)) || (changes.length == 0)) {
                        event.nolog = 1;
                    } // If this is a temporary device, don't log changes
                    if (db.changeStream) {
                        event.noact = 1;
    ...
    function ChangeAgentLocationInfo(command) {
        ...
        if (obj.deviceChanging === true) {
            setTimeout(function () {
                ChangeAgentLocationInfo(command);
            }, 100);
            return;
        }
        obj.deviceChanging = true;
        ...
        db.Get(obj.dbNodeKey, function (err, nodes) {
            ...
            if (device.agent) {
                ...
                // Check if anything changes
                if ((command.publicip) && (device.publicip != command.publicip)) {
                    device.publicip = command.publicip;
                    change = 1;
                    changes.push('public ip');
                }
                if ((command.iploc) && (device.iploc != command.iploc)) {
                    device.iploc = command.iploc;
                    change = 1;
                    changes.push('ip location');
                }
                ...
                if (change == 1) {
                    ...
                    var event = {
                        ...
                        node: parent.CloneSafeNode(device),
                        msgid: 59,
                        msgArgs: [device.name, mesh.name, changes.join(', ')],
                        msg: 'Changed device ' + device.name + ' from group ' + mesh.name + ': ' + changes.join(', ')
                    };
                    if (obj.agentInfo.capabilities & 0x20) {
                        event.nolog = 1;
                    } // If this is a temporary device, don't log changes
    ...
    function ChangeAgentTag(tag) {
        ...
        // If the device is pending a change, hold.
        if (obj.deviceChanging === true) {
            var func = function ChangeAgentTagFunc() {
                ChangeAgentCoreInfo(ChangeAgentTagFunc.tag);
            }
            func.tag = tag;
            setTimeout(func, 100);
            return;
        ...
        db.Get(obj.dbNodeKey, function (err, nodes) {
            ...
            if (device.agent) {
                    ...
                    for (var i in taglines) {
                        ...
                        if (tagline.length > 0) {
                            if (tagline.startsWith('~')) {
                                if (tagline.startsWith('~ServerName:') && (tagline.length > 12) && (serverName == null)) {
                                    serverName = tagline.substring(12).trim();
                                }
                                if (tagline.startsWith('~ServerDesc:') && (tagline.length > 12) && (serverDesc == null)) {
                                    serverDesc = tagline.substring(12).trim();
                                }
                                if (tagline.startsWith('~ServerTags:') && (tagline.length > 12) && (serverTags == null)) {
                                    serverTags = tagline.substring(12).split(',');
                                    for (var j in serverTags) {
                                        serverTags[j] = serverTags[j].trim();
                                    }
                                }
                            } else {
                                if (agentTag == null) {
                ...
                // Set the agent tag
                var changes = false;
                if (device.agent.tag != agentTag) {
                    device.agent.tag = agentTag;
                    if ((device.agent.tag == null) || (device.agent.tag == '')) {
                        delete device.agent.tag;
                    }
                    changes = true;
                }
                if (domain.agenttag != null) {
                    // Set the device's server name
                    if ((serverName != null) && (domain.agenttag.servername === 1) && (device.name != serverName)) {
                        device.name = serverName;
                        changes = true;
                    }

                    // Set the device's server description
                    if ((serverDesc != null) && (domain.agenttag.serverdesc === 1) && (device.desc != serverDesc)) {
                        device.desc = serverDesc;
                        changes = true;
                    }

                    // Set the device's server description if there is no description
                    if ((serverDesc != null) && (domain.agenttag.serverdesc === 2) && (device.desc != serverDesc) && ((device.desc == null) || (device.desc == ''))) {
                        device.desc = serverDesc;
                        changes = true;
                    }
                    ...
                    if ((serverTags != null) && (domain.agenttag.servertags != null) && (domain.agenttag.servertags != 0)) {
                        ...
                        if (device.tags != null) {
                            st2 = device.tags.join(',');
                        }

                        // Set the device's server tags
                        if ((domain.agenttag.servertags === 1) && (st1 != st2)) {
                            device.tags = serverTags;
                            changes = true;
                        }

                        // Set the device's server tags if there are not tags
                        if ((domain.agenttag.servertags === 2) && (st2 == '')) {
                            device.tags = serverTags;
                            changes = true;
                        }
                        ...
                        if ((domain.agenttag.servertags === 3) && (st1 != st2)) {
                            ...
                            for (var i in serverTags) {
                                if (device.tags.indexOf(serverTags[i]) == -1) {
                                    device.tags.push(serverTags[i]);
                                }
                            }
                            device.tags.sort();
                            changes = true;
    ...
    obj.RequestCoreDump = function (agenthashhex, corehashhex) {
        if (agenthashhex.length > 16) {
            agenthashhex = agenthashhex.substring(0, 16);
        }
        const cookie = parent.parent.encodeCookie({
            a: 'aft',
            b: 'coredump',
            c: obj.agentInfo.agentId + '-' + agenthashhex + '-' + obj.nodeid + '.dmp'
        }, parent.parent.loginCookieEncryptionKey);
